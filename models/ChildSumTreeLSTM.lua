...
function ChildSumTreeLSTM:backward(tree, inputs, grad)
  local grad_inputs = torch.CudaTensor(inputs:size())
...
function ChildSumTreeLSTM:get_child_states(tree)
  ...
  if tree.num_children == 0 then
    child_c = torch.CudaTensor(1, self.mem_dim):zero()
    child_h = torch.CudaTensor(1, self.mem_dim):zero()
  else
    child_c = torch.CudaTensor(tree.num_children, self.mem_dim)
    child_h = torch.CudaTensor(tree.num_children, self.mem_dim)
    for i = 1, tree.num_children do
