...
function LSTM:__init(config)
  ...
  if self.num_layers == 1 then
    ctable_init = torch.CudaTensor(self.mem_dim):zero()
    htable_init = torch.CudaTensor(self.mem_dim):zero()
    ctable_grad = torch.CudaTensor(self.mem_dim):zero()
    htable_grad = torch.CudaTensor(self.mem_dim):zero()
  else
    ctable_init, ctable_grad, htable_init, htable_grad = {}, {}, {}, {}
    for i = 1, self.num_layers do
      ctable_init[i] = torch.CudaTensor(self.mem_dim):zero()
      htable_init[i] = torch.CudaTensor(self.mem_dim):zero()
      ctable_grad[i] = torch.CudaTensor(self.mem_dim):zero()
      htable_grad[i] = torch.CudaTensor(self.mem_dim):zero()
    end
  end
  self.initial_values = {ctable_init, htable_init}
  self.gradInput = {
    torch.CudaTensor(self.in_dim):zero(),
    ctable_grad,
    htable_grad
  }
end
...
function LSTM:backward(inputs, grad_outputs, reverse)
  ...
  local input_grads = torch.CudaTensor(inputs:size())
  for t = size, 1, -1 do
    local input = reverse and inputs[size - t + 1] or inputs[t]
