...
function treelstm.read_relatedness_dataset(dir, vocab, model, dataset_name, is_blind_set)
  local dataset = {}
  dataset.vocab = vocab
  if model =='constituency' then
    print('reading ' .. dir .. ' constituency trees ... ')
    dataset.ltrees = treelstm.read_trees(dir .. 'a.cparents')
    dataset.rtrees = treelstm.read_trees(dir .. 'b.cparents')
  elseif model == 'dependency' then
    print('reading ' .. dir .. ' dependency trees ... ')
    dataset.ltrees = treelstm.read_trees(dir .. 'a.parents')
    dataset.rtrees = treelstm.read_trees(dir .. 'b.parents')
  end
  print('reading ' .. dir .. ' sentences ... ')
  dataset.lsents = treelstm.read_sentences(dir .. 'a.toks', vocab) 
  dataset.rsents = treelstm.read_sentences(dir .. 'b.toks', vocab)
  
  if model == 'dependency' or model == 'constituency' then
    dataset.size = #dataset.ltrees
    if #dataset.ltrees ~= #dataset.rtrees then
      error('left and right sentences differ in size')
    end
  else
    dataset.size = #dataset.lsents
    if #dataset.lsents ~= #dataset.rsents then
      error('left and right sentences differ in size')
    end
  end
  ...
  if is_blind_set == false then
    ...
    for i = 1, dataset.size do
      --dataset.labels[i] = sim_file:readInt() + 1 
