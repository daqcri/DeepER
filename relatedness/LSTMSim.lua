...
function LSTMSim:__init(config)
  ...
  -- KL divergence optimization objective
   self.weights = torch.Tensor(2)
   self.weights[1] = 0.997
   self.weights[2] = 0.003
   self.criterion = nn.ClassNLLCriterion(self.weights, true, -100)
...
function LSTMSim:predict_dataset(dataset,confusion, is_blind_set,do_save, to_save_data, to_save_labels)
  ...
  for i = 1, dataset.size do
    ...
    if do_save == true then
      if is_blind_set == false then
        to_save_data[i] = torch.cat(inputs[1]:float(),inputs[2]:float())
        to_save_labels[i] = sim
      else
        to_save_data[i] = torch.cat(inputs[1]:float(),inputs[2]:float())
      end
    end
