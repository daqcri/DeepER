...
function LSTMSim:__init(config)
  ...
  if self.structure == 'lstm' or self.structure == 'avg' then
    local xl = nn.Identity()()
    local xls = nn.SplitTable(1)(xl)
    xlsum = nn.CAddTable()(xls)
    self.lmean = nn.gModule({xl}, {xlsum})
    self.lmean:cuda()

    local xr = nn.Identity()()
    local xrs = nn.SplitTable(1)(xr)
    xrsum = nn.CAddTable()(xrs)
    self.rmean = nn.gModule({xr}, {xrsum})
    self.rmean:cuda()
...
function LSTMSim:new_sim_module()
  ...
  if self.structure == 'avg' then
    input_dim = 2 * self.num_layers * self.emb_dim
  ...
  elseif self.structure == 'lstm'  then 
...
function LSTMSim:train(dataset, leave_out, noise_level)
  self.llstm:training()
  self.rlstm:training()
  local num_to_noise = math.floor((dataset.size-(leave_out[2]-leave_out[1]))*noise_level)
  if self.structure == 'bilstm' then
  ...
  for i = 1, dataset.size, self.batch_size do
    if (i < leave_out[1]) or (i> leave_out[2])then 
     ...
     -- get target distributions for batch
      ...
      local feval = function(x)
        ...
        for j = 1, batch_size do
          ...
          local inputs
          if self.structure == 'avg' then
            inputs = {self.lmean:forward(linputs)/linputs:size(1), self.rmean:forward(rinputs)/rinputs:size(1)}
          elseif self.structure == 'lstm' then
          ...
          if self.structure == 'avg' then
            linput_grads = self.lmean:backward(linputs,rep_grad[1])
            rinput_grads = self.rmean:backward(rinputs,rep_grad[2])
          elseif self.structure == 'lstm' then
      ...
      if self.optim_method == 'adam' then
        optim.adam(feval, self.params, self.optim_state)
      elseif self.optim_method == 'adagrad' then
...
function LSTMSim:predict_dataset(dataset, leave_out, is_dev, confusion, is_blind_set)
  local predictions = torch.CudaTensor(dataset.size,2)
  local inputs = nil
  local sim = nil
  local eval_span = {}
  if is_dev == true then
    for k = leave_out[1], leave_out[2] do
      table.insert(eval_span,k)
    end
  else
    for k = 1, (leave_out[1]-1) do
      table.insert(eval_span, k)
    end
    for k = (1+leave_out[2]), dataset.size do
      table.insert(eval_span, k)
    end
  end 
  for k,i in pairs(eval_span) do
    xlua.progress(i, eval_span[#eval_span])
    local lsent, rsent = dataset.lsents[i], dataset.rsents[i]
    predictions[i],inputs = self:predict(lsent, rsent)
  ...
  return {predictions, precision, recall, f1_score}
end
