...
Training script for semantic relatedness prediction on the SICK dataset.
  ...
  -z,--noise_level (default 0) noise ratio in training data
  -t,--op_mode (default train_test) Train or Test Mode: [train_test, test]
  -b,--saved_model_path (default Nope)
]]
...
local file_idx = 1
while true do
  debug_file_path = string.format(debug_dir .. 'rel-%s.%dl.%dd.%dsimd.%.5flr.%.5freg.%swup.%5flrw.%sopt.%dnoise.%d.debug', args.model, args.layers, args.dim, args.sim_nhidden,args.learning_rate, args.regularization, args.update_emb, args.emb_learning_rate, args.optim_method,args.noise_level,file_idx)
...
local file_idx = 1
while true do
  train_save_data_path = string.format(data_dir .. 'rel-%s.%dl.%dd.%dsimd.%.5flr.%.5freg.%swup.%5flrw.%sopt.%dnoise.%d.%s.t7', args.model, args.layers, args.dim, args.sim_nhidden,args.learning_rate, args.regularization, args.update_emb, args.emb_learning_rate, args.optim_method,args.noise_level,file_idx,'train_data')
  dev_save_data_path = string.format(data_dir .. 'rel-%s.%dl.%dd.%dsimd.%.5flr.%.5freg.%swup.%5flrw.%sopt.%dnoise.%d.%s.t7', args.model, args.layers, args.dim, args.sim_nhidden,args.learning_rate, args.regularization, args.update_emb, args.emb_learning_rate, args.optim_method,args.noise_level,file_idx,'dev_data')
  test_save_data_path = string.format(data_dir .. 'rel-%s.%dl.%dd.%dsimd.%.5flr.%.5freg.%swup.%5flrw.%sopt.%dnoise.%d.%s.t7', args.model, args.layers, args.dim, args.sim_nhidden,args.learning_rate, args.regularization, args.update_emb, args.emb_learning_rate, args.optim_method,args.noise_level,file_idx,'test_data')

  train_save_labels_path = string.format(data_dir .. 'rel-%s.%dl.%dd.%dsimd.%.5flr.%.5freg.%swup.%5flrw.%sopt.%dnoise.%d.%s.t7', args.model, args.layers, args.dim, args.sim_nhidden,args.learning_rate, args.regularization, args.update_emb, args.emb_learning_rate, args.optim_method,args.noise_level,file_idx,'train_labels')
  dev_save_labels_path = string.format(data_dir .. 'rel-%s.%dl.%dd.%dsimd.%.5flr.%.5freg.%swup.%5flrw.%sopt.%dnoise.%d.%s.t7', args.model, args.layers, args.dim, args.sim_nhidden,args.learning_rate, args.regularization, args.update_emb, args.emb_learning_rate, args.optim_method,args.noise_level,file_idx,'dev_labels')
  test_save_labels_path = string.format(data_dir .. 'rel-%s.%dl.%dd.%dsimd.%.5flr.%.5freg.%swup.%5flrw.%sopt.%dnoise.%d.%s.t7', args.model, args.layers, args.dim, args.sim_nhidden,args.learning_rate, args.regularization, args.update_emb, args.emb_learning_rate, args.optim_method,args.noise_level,file_idx,'test_labels')
...
local numNoisedPositive = 0
local numNoisedNegative = 0

local numPositive = 0
local numNegative = 0

for i = 1, train_dataset.size do
  if train_dataset.labels[i] == 1 then
...
if args.noise_level > 0 then
  local numToNoisePositive = math.floor(args.noise_level * numPositive)
  local numToNoiseNegative = math.floor(args.noise_level * numNegative)
  local indices = torch.randperm(train_dataset.size)
  local i = 0
  while true do
    if (numNoisedPositive >= numToNoisePositive) and (numNoisedNegative >= numToNoiseNegative) then
      print('here ...')
      break
    end
    i = i % train_dataset.size + 1
    local idx = indices[i]
    if train_dataset.labels[idx] == 1 and numNoisedPositive < numToNoisePositive then
      train_dataset.labels[idx] = 2
      numNoisedPositive = numNoisedPositive + 1
    elseif train_dataset.labels[idx] == 2 and numNoisedNegative < numToNoiseNegative then
      train_dataset.labels[idx] = 1
      numNoisedNegative = numNoisedNegative + 1
    end
  end
end
...
file_idx = 1
while true do
  perf_file_path = string.format(perf_dir .. '/rel-%s.%dl.%dd.%dsimd.%.5flr.%.5freg.%swup.%.5flrw.%sopt.%.5fnoise.%d.perf', args.model, args.layers, args.dim, args.sim_nhidden,args.learning_rate, args.regularization, args.update_emb,args.emb_learning_rate, args.optim_method,args.noise_level,file_idx)
...
-- local do_save = false
if args.op_mode == 'train_test' then 
  ...
  for i = 1, num_epochs do
    ...
    --   do_save = true
    -- end
    model:train(train_dataset,saveData,data_to_save)
...
if args.op_mode == 'test' then
  header('Testing Mode')
  best_dev_model = model_class.load(args.saved_model_path)
end
-- evaluate
...
while true do
  predictions_save_path = string.format(perf_dir .. '/rel-%s.%dl.%dd.%dsimd.%.5flr.%.5freg.%swup.%.5flrw.%sopt.%.5dnoise.%d.pred', args.model, args.layers, args.dim, args.sim_nhidden,args.learning_rate, args.regularization, args.update_emb, args.emb_learning_rate, args.optim_method,args.noise_level,file_idx)
  model_save_path =             string.format(perf_dir .. '/rel-%s.%dl.%dd.%dsimd.%.5flr.%.5freg.%swup.%.5flrw.%sopt.%.5fnoise.%d.model', args.model, args.layers, args.dim, args.sim_nhidden,args.learning_rate, args.regularization, args.update_emb, args.emb_learning_rate, args.optim_method,args.noise_level,file_idx)
